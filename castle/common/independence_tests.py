...
from .Tigramite_Stuff import cmisymb
...
class CITest(object):
    ...
    def CMI_symb(data, x, y, z):
        ...
        cmi = cmisymb.CMIsymb(n_symbs=4, sig_samples=1000,
                        significance='shuffle_test', sig_blocklength=1,
                        conf_blocklength=1)
        
        MI, p_value = cmi.run_test_raw(X, Y, z=Z)
        print(x,y,z,MI, p_value)
        return None, None, p_value

    @staticmethod
    def CMI_Knn_test(data, x, y, z):
        X = data[:,x].reshape(data.shape[0],1)
        Y = data[:,y].reshape(data.shape[0],1)
        if len(z)==0:
            Z = None
        ...
        cmi = cmiknn.CMIknn(knn=0.15, shuffle_neighbors=10, sig_samples=50,
    ...
    @staticmethod
    def partial_correlation_spearman_test(data, x, y, z):
        df = pd.DataFrame(data)
        parr_corr = pg.partial_corr(data=df, x=x, y=y, covar=z, method = "spearman")
        p_value = parr_corr.iloc[0]["p-val"]
        r = parr_corr.iloc[0]["r"]
        print(x,y,z,r, p_value)
        return None, None, p_value
